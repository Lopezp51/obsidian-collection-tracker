...
Utilize um script Python nativo no ambiente (ou sua varredura interna) para encontrar arquivos no Obsidian nas pastas "Gestão de Compras" ou "Acervo" cuja chave property do YAML `Equipe Criativa Geral` não existe, está vazia (`null`), ou é uma lista vazia (`[]`).
...
Processe os arquivos usando o módulo `yaml` de Python manipulando especificamente o Header de forma segura (preservando o resto da nota). Atualize as respectivas 10 notas injetando o pacote preparado dentro de `Equipe Criativa Geral`.
